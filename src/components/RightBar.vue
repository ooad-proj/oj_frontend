<template>
  <v-navigation-drawer v-model="shown" app right absolute temporary>
    <div v-if="isLogin" class=" tw-flex tw-flex-wrap tw-h-full tw-content-between">
      <div class=" tw-w-full">
      ...
      <v-list class=" tw-w-full">
        <v-list-item @click="logout()" class=" tw-transition-all hover:tw-bg-red-100 tw-cursor-default" v-ripple="{class:`error--text`}">
    ...
    <div v-else class=" tw-flex tw-flex-wrap tw-h-full tw-justify-center tw-content-center">
      <div>
        <p class=" tw-text-center tw-font-bold">未登录</p>
        <v-btn class=" teal white--text" @click="$router.push('/login')">登录</v-btn>
      </div>
    </div>
  </v-navigation-drawer>
</template>
